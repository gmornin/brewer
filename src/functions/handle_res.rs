...
pub fn ev1_handle(err: &V1Error) -> Result<(), Box<dyn Error>> {
    ...
    match err {
        ...
        V1Error::InvalidToken => {
            ...
            tokio::task::block_in_place(|| creds.save())?;
...
pub fn v1_handle(res: &V1Response) -> Result<(), Box<dyn Error>> {
    ...
    match res {
        ...
        V1Response::Exists { value: true } => println!("The requested file item does exist."),
        V1Response::Exists { value: false } => println!("The requested file item does not exist."),
