...
pub fn ignore_tree(path: &Path) -> V1DirTreeNode {
    ...
    V1DirTreeNode {
        ...
        content: ignore_tree_recurse(path, GitignoreBuilder::new(path)),
    }
}

pub fn ignore_tree_recurse(current: &Path, mut builder: GitignoreBuilder) -> V1DirTreeItem {
    trace!(
        "Fs tree tracing in `{}`",
    ...
    let gmignore = current.join(".gmignore");
    if gmignore.exists() {
    ...
    current.read_dir().unwrap().for_each(|entry| {
        let entry = entry.unwrap();
        let metadata = entry.metadata().unwrap();

        if ignores.matched(entry.path(), metadata.is_dir()).is_ignore() {
            return;
        }
        ...
        entries.push(V1DirTreeNode {
            ...
            content: if metadata.is_file() {
                ...
                ignore_tree_recurse(current.join(entry.path()).as_path(), builder.clone())
            },
        });
    });
